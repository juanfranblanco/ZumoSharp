...
namespace Zumo.Sharp.Authentication
{
    public class JsonWebToken
    {
        ...
        public class JsonWebTokenClaims
        {
            ...
            public JsonWebTokenClaims(double expUnixTime, string issuer, string audience, string userId, int version, string clientIdentifier, string appId)
            {
                this.expUnixTime = expUnixTime;
                Issuer = issuer;
                Audience = audience;
                UserId = userId;
                Version = version;
            ...
            [DataMember(Name = "urn:microsoft:credentials", Order=6)]
            public string CryptedCredentials
            {
                get;
                internal set;
            }
        ...
        {
            public JsonWebTokenEnvelope()
            {
            }

            public JsonWebTokenEnvelope(string type, string algorithm, int keyId)
            {
                Type = type;
                Algorithm = algorithm;
                KeyId = keyId;
        ...
        public string EnvelopeTokenSegment
        {
            get { return envelopeTokenSegment; }
        }

        public string ClaimsTokenSegment
        {
            get { return claimsTokenSegment; }
        }
        ...
        #region Constructors
        public JsonWebToken(string token, string key)
        {
            ...
            this.Claims.Credentials = this.DecryptCredentials(Claims.CryptedCredentials, key);
        ...
        {
            return this.EnvelopeTokenSegment + "." + this.ClaimsTokenSegment + "." + this.Signature;
        }

        private string EncryptCredentials(dynamic credentials, string key)
        {
            var plaintext = JsonConvert.SerializeObject(credentials);
            return CryptoUtility.Encrypt(plaintext, key);
        }

        private dynamic DecryptCredentials(dynamic credentials, string key)
        {
            var stringDecrypted =  CryptoUtility.Decrypt(credentials, key);
            return JsonConvert.DeserializeObject<dynamic>(stringDecrypted);
        }
        ...
        private static string GenerateJsonString(DataContractJsonSerializer serializer, object input)
        {
        ...
        private JsonWebTokenClaims GetClaimsFromTokenSegment(string claimsTokenSegment)
        {
            ...
            string s = Encoding.UTF8.GetString(claimsData, 0, claimsData.Length);
            Debug.WriteLine(s);
        ...
        {
            var base64UrlEncodedHash = BuildSignature(key);
		...
		private string BuildSignature(string key)
        {
